...
function dbg(msg) { _outputChannel?.appendLine('[JarvisPanel] ' + msg); }
class JarvisViewProvider {
    ...
    async _loadAllSessions() {
        try {
            ...
            const jobHistory = histData?.['jobs'] ?? [];
    ...
    async _deleteSession(sessionId, workspaceRoot, isGlobal) {
        try {
    ...
    async _renameSession(name) {
        if (!this._currentSessionId || !name) {
            return;
        }
        try {
    ...
    async _exportSession(sessionId, workspaceRoot, isGlobal) {
        try {
            ...
            const data = await res.json();
            const defaultUri = vscode.Uri.file((workspaceRoot || (vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? '')) +
    ...
    async _searchSessions(query, workspaceRoot) {
        if (!query.trim()) {
    ...
    async sessionQuickPick() {
        try {
            ...
            const data = await res.json();
            const items = [];
            const addSessions = (sessions, wsRoot, isGlobal) => {
                sessions.forEach(s => {
                    const msgs = s['messages'] ?? [];
                    items.push({
                        label: s['name'] || '(untitled)',
                        description: `${msgs.length} msg${msgs.length !== 1 ? 's' : ''}`,
                        detail: isGlobal ? 'Global' : (s['project_name'] || wsRoot.split(/[\\/]/).pop() || wsRoot),
                        sessionId: s['id'],
                        wsRoot,
                        isGlobal,
                    });
            ...
            addSessions(data['global'] ?? [], '', true);
            const projects = data['projects'] ?? {};
            Object.entries(projects).forEach(([ws, proj]) => {
                const p = proj;
                addSessions(p['sessions'] ?? [], ws, false);
            });
...
JarvisViewProvider.viewType = 'jarvis.view';
